...
import { GetCarInfo } from "../types/carInfo.ts"
import { Trim } from "../types/trims.ts"
import { useParamMutation } from "../hooks/useParamMutation.ts"
import { useMakes } from "../hooks/useMakes.ts"
import { Model } from "../types/models.ts"
import { useModelsMutation } from "../hooks/useModelsMutation.ts"
import { useTrimMutation } from "../hooks/useTrimMutation.ts"

export const generateRandomCar = () => {
  const [selectedMake, setSelectedMake] = useState("")
  const [selectedModel, setSelectedModel] = useState("")
  const [selectedTrim, setSelectedTrim] = useState<Trim | undefined>()
  const [randomCar, setRandomCar] = useState<GetCarInfo[]>([])

  const paramMutation = useParamMutation(setRandomCar)
